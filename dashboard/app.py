...
- All static assets (checkpoints, precomputed C&W gallery, CSVs, demo
  images) live directly under dashboard/assets/ -- checkpoints are NOT
  in a separate subfolder. See README_DASHBOARD.md for the two one-time
  Kaggle scripts that generate the assets this app cannot compute live
  (demo image subset + C&W gallery).
...
def load_model(filename: str):
    # Checkpoints are committed directly into dashboard/assets/, not a
    # checkpoints/ subfolder -- matches how they're actually laid out
    # in the repo.
...
with tab_attack:
    ...
    if images is None:
        ...
        if model_std is None or model_rob is None:
            missing_asset_notice("resnet18_*.pt")
        else:
            gallery_note = None
